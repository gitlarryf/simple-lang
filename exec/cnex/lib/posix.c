...
{
    char name[256];
    Cell *r = wrap(gethostname(name, 256));
...
{
    char *new_ = string_asCString(top(exec->stack)->string); pop(exec->stack);
    char *old_ = string_asCString(top(exec->stack)->string); pop(exec->stack);

    push(exec->stack, wrap(rename(old_, new_)));
    free(new_);
    free(old_);
}
