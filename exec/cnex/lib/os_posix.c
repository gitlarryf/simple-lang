...
{
    if (pp == NULL || pp->ptr == NULL || ((TProcessHandle*)pp->ptr)->proc == 0) {
...
void object_releaseProcessObject(Object *o)
{
    if (o != NULL) {
        ...
        if (o->refcount <= 0) {
            if (o->ptr != NULL) {
                if (((TProcessHandle*)o->ptr)->proc != 0) {
                    close(((TProcessHandle*)o->ptr)->proc);
                }
                free(o->ptr);
            }
            free(o);
...
{
    Cell *r = cell_fromCString("<PROCESS ");
    char s[16];
    sprintf(s, "%d", (pid_t)((TProcessHandle*)self->ptr)->proc);
    r->string = string_appendCString(r->string, s);
