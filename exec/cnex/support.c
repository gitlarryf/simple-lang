...
void path_initPaths(const char *source_path)
{
    ...
    if (ne != NULL) {
        ...
        // Push the last (or perhaps only) path in the list to the list of paths, if it's not blank.
        if (path != NULL && *path != '\0') {
            path_pushPath(path);
        }
    }
...
char *path_getPathOnly(char *path)
{
    ...
    //e++; // Step one beyond any path separator, even if we didn't find one.
